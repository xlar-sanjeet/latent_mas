...
def main():
    ...
    parser.add_argument(
        ...
        default=1e-5,
        help=(
            "Regularization coefficient used when generating "
            "the latent realignment matrix."
        ),
    )
    parser.add_argument(
        "--realign_chunk_size",
        ...
        default=4096,
        help=(
            "Number of vocabulary rows processed per chunk "
            "while generating the realignment matrix."
        ),
    )
    ...
    if args.realign_regularization <= 0:
        raise ValueError("--realign_regularization must be positive.")
    if args.realign_chunk_size <= 0:
        raise ValueError("--realign_chunk_size must be positive.")

    # The current LatentMAS vLLM latent-insertion path is Qwen-specific.
    model_name_lower = args.model_name.lower()
    if args.use_vllm and "llama" in model_name_lower:
        raise ValueError(
            "The current LatentMAS vLLM latent-insertion path is "
            "Qwen-specific. Run Llama through the Hugging Face "
            "backend by removing --use_vllm."
        )
    ...
    # --think uses a Qwen-specific convention and must not be applied to other families.
    if args.think and model.model_type != "qwen3":
        raise ValueError(
            "--think currently supports Qwen-style models only."
        )
