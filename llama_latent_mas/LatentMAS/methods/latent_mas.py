...
class LatentMASMethod:
    def __init__(
        ...
        *,
        ...
        # vLLM is optional: only build sampling params when the vLLM backend is used.
        if bool(getattr(args, "use_vllm", False)):
            if SamplingParams is None:
                raise ImportError(
                    "--use_vllm was set but vllm is not installed."
                )
    ...
    def _truncate_past(self, past_kv: Optional[Any], tokens_to_keep: int) -> Optional[Any]:
    ...
    @staticmethod
    def _truncate_attention_mask(
        mask: Optional[torch.Tensor], tokens_to_keep: int
    ) -> Optional[torch.Tensor]:
        # Truncate the attention mask identically to the KV cache so that
        # preserved padding zeros remain aligned with the retained cache positions.
        if mask is None or tokens_to_keep <= 0:
            return None
    ...
    def run_batch(self, items: List[Dict]) -> List[Dict]:
        ...
        for agent in self.agents:
            ...
            if agent.role != "judger":
                ...
                past_kv, past_attention_mask = self.model.generate_latent_batch(
                    wrapped_ids,
                ...
                mask_for_decoding = (
                    past_attention_mask if self.latent_steps > 0 else None
                )
                ...
                generated_batch, _ = self.model.generate_text_batch(
                    ...
                    past_attention_mask=mask_for_decoding,
                )
                for idx in range(batch_size):
