...
# Override HF_HOME early (before any HuggingFace imports) if it points to a non-writable placeholder
_default_hf_cache = os.path.join(os.path.dirname(os.path.abspath(__file__)), "hf_cache")
if not os.environ.get("HF_HOME") or os.environ["HF_HOME"] in ("/datadisk/Latent_MAS_Kritika/LatentMAS/hf_cache", ""):
    os.environ["HF_HOME"] = _default_hf_cache
# Allow --cache_dir CLI arg to override (parsed early via sys.argv)
for _i, _arg in enumerate(sys.argv):
    if _arg == "--cache_dir" and _i + 1 < len(sys.argv):
        os.environ["HF_HOME"] = sys.argv[_i + 1]
        break
...
def main():
    ...
    parser.add_argument("--model_name", type=str, required=True,
                        help="HuggingFace model name to use (e.g. 'Qwen/Qwen3-14B', 'mistralai/Mistral-7B-Instruct-v0.3').")
    ...
    parser.add_argument("--cache_dir", type=str, default=None, help="HuggingFace cache directory for datasets and models")
    ...
    if args.task == "gsm8k":
        dataset_iter = load_gsm8k(split=args.split, cache_dir=cache_dir)
    elif args.task == "aime2024":
        dataset_iter = load_aime2024(split="train", cache_dir=cache_dir)
    elif args.task == "aime2025":
        dataset_iter = load_aime2025(split='train', cache_dir=cache_dir)
    elif args.task == "gpqa":
        dataset_iter = load_gpqa_diamond(split='test', cache_dir=cache_dir)
    elif args.task == "arc_easy":
        dataset_iter = load_arc_easy(split='test', cache_dir=cache_dir)
    elif args.task == "arc_challenge":
        dataset_iter = load_arc_challenge(split='test', cache_dir=cache_dir)
    elif args.task == "mbppplus":
        dataset_iter = load_mbppplus(split='test', cache_dir=cache_dir)
    elif args.task == "humanevalplus":
        dataset_iter = load_humanevalplus(split='test', cache_dir=cache_dir)
    elif args.task == "medqa":
        dataset_iter = load_medqa(split='test', cache_dir=cache_dir)
    else:
