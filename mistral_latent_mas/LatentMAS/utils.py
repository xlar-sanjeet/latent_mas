...
# extract multiple-choice answer (A/B/C/D) from \boxed{} or last occurrence in text
def extract_mc_answer(text: str) -> Optional[str]:
    boxes = re.findall(r"\\boxed\{([^}]*)\}", text)
    if boxes:
        content = boxes[-1].strip()
        letter = re.search(r"[A-Da-d]", content)
        return letter.group(0) if letter else content
    # fallback: last standalone A/B/C/D letter in text
