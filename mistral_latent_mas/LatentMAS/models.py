...
def _past_length(past_key_values: Optional[Tuple]) -> int:
    if not past_key_values:
        return 0
    # Handle DynamicCache and other Cache objects (transformers >= 4.38)
...
class ModelWrapper:
    ...
    def get_user_content_marker(self, rendered_prompt: str, user_content: str) -> str:
        """Return the prefix of rendered_prompt up to (but not including) user_content,
        so callers can measure how many tokens precede the user's question."""
        model_name_lower = self.model_name.lower()
        if "qwen" in model_name_lower:
            marker = "<|im_start|>user\n"
            idx = rendered_prompt.find(marker)
            if idx != -1:
                return rendered_prompt[: idx + len(marker)]
        # For Mistral and other models: locate user content directly in the rendered prompt.
        idx = rendered_prompt.find(user_content)
        if idx != -1:
            return rendered_prompt[:idx]
