...
from utils import extract_gsm8k_answer, extract_mc_answer, normalize_answer, extract_markdown_python_block, run_with_timeout
import torch
import argparse
    ...
    def run_batch(self, items: List[Dict]) -> List[Dict]:
        ...
        for idx, item in enumerate(items):
            ...
            if self.task in ['mbppplus', 'humanevalplus']:
                ...
                if self.task in ["arc_easy", "arc_challenge", "gpqa", "medqa"]:
                    pred = normalize_answer(extract_mc_answer(final_text))
                else:
                    pred = normalize_answer(extract_gsm8k_answer(final_text))
                gold = item.get("gold", "")
    ...
    def run_batch_vllm(self, items: List[Dict]) -> List[Dict]:
        ...
        for agent in self.agents:
            ...
            if agent.role != "judger":
                ...
                # handle latent embedding insertion position (model-agnostic)
                len_of_left = []
                for p, item in zip(judger_prompts, items):
                    user_content = item["question"]
                    left = self.model.get_user_content_marker(p, user_content)
